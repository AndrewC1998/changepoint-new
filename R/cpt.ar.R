cpt.ar <- function(data, penalty = "MBIC", pen.value = 0, min.order = 1, max.order=5, method = "PELT", dist = "Normal", class = TRUE, param.estimates = TRUE, minseglen = 1, shape = 0, tol = 1e-07){
  ...
  pen.value <- penalty_decision(penalty=penalty, pen.value=pen.value, n=nrow(sumstat), diffparam=ncol(sumstat), asymcheck="cpt.reg", method=method)
  
  answer=list()
  answer[[6]]=1
  on.exit(.C("FreePELT",answer[[6]]))
  
  answer <- .C('PELT', cost_func=cost_func, sumstat=as.double(sumstat), n=as.integer(n), m=as.integer(length(sumstat[1,])), pen=as.double(pen.value), cptsout=vector("integer",n), error=as.integer(0), shape=as.double(shape), minorder=as.integer(min.order), optimalorder = as.integer(0), maxorder = as.integer(max.order), minseglen=as.integer(minseglen), tol=as.double(tol), lastchangelike=vector("double", n+1), lastchangecpts=vector("integer", n+1), numchangecpts=vector("integer", n+1), MBIC=as.integer(MBIC))
  ...
  if(class==TRUE){
      ...
      return(list(order = answer$optimalorder, lastchangecpts=answer$lastchangecpts,
